...
import type { PersonTableRows, CommentTableRows } from '../types/structures.js';
...
export default (): ExpressType => {
  ...
  app.get('/persons/phone/:phone', async (req, res, next) => {
    ...
    try {
      const data = (await getPersonByPhone(phone)) as PersonTableRows;
      if (data.length === 0) {
  ...
  app.get('/persons/vk/:vk', async (req, res, next) => {
    ...
    try {
      const data = (await getPersonByVK(url.pathname)) as PersonTableRows;
      if (data.length === 0) {
        return next(new NoDataError());
      }
  ...
  app.get('/persons/firstname/:firstname/lastname/:lastname', async (req, res, next) => {
    ...
    try {
      const data = (await getPersonByName(firstname, lastname)) as PersonTableRows;
      if (data.length === 0) {
        return next(new NoDataError());
      }
  ...
  app.get('/persons/count', async (req, res, next) => {
    try {
      const count = (await getCountOfPersons()) as number;
  ...
  app.get('/comments/:personId', async (req, res, next) => {
    ...
    try {
      const data = (await getCommentById(Number(personId))) as CommentTableRows;
      if (data.length === 0) {
        return next(new NoDataError());
      }
  ...
  app.use(
    (() => {
      const fn: ErrorHandleFunction = (err, req, res, next) => {
